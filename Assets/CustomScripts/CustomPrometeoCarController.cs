...
namespace CarControllerMain
{
    public enum DriveType { FWD, RWD, AWD }

    public class CustomPrometeoCarController : MonoBehaviour
    {

        #region CAMERA DATA
        public new CinemachineCamera[] camera;
        [SerializeField] private GameObject currentCamera;
        private CinemachineFollow currentCamFollow;
        ...
        */
        ...
        [Header("CONTROLS")]
        [Space(10)]
        CustomDefaultActions inputActions;
        #endregion
        #region CAR DATA
        ...
        */
        ...
        float gasInput, brakeInput, handbrakeInput, steeringInput;
        TrackCheckpoints _trackCheckpoints;
        #endregion

        private void Awake()
        {
            _trackCheckpoints = FindFirstObjectByType<TrackCheckpoints>();

            inputActions = new CustomDefaultActions();

            camera = FindObjectsByType<CinemachineCamera>(FindObjectsSortMode.InstanceID);
        ...
        void Start()
        {
            ...
            inputActions.CarPlaying.ChangeCamera.performed += ChangeCamera;

            Array.ForEach(camera, i => i.gameObject.SetActive(false));

            currentCamera = camera[camIndex].gameObject;
            currentCamera.SetActive(true);

            currentCamFollow = currentCamera.GetComponent<CinemachineFollow>();
        ...
        private void Update()
        {
            handbrakeInput = inputActions.CarPlaying.HandBrake.ReadValue<float>();
            steeringInput = inputActions.CarPlaying.Turn.ReadValue<float>();

            Turn(steeringInput);

            if (steeringInput == 0 && steeringAxis != 0f)
            {
                ResetSteeringAngle();
            }

            if (!_trackCheckpoints.isRacing)
            {
            ...
            //Debug.Log($"speed: {carSpeed}, gas: {gasInput}, brake: {brakeInput}, handbrake: {handbrakeInput}, steering: {steeringInput}");
        }
        void FixedUpdate()
        {
            gasInput = inputActions.CarPlaying.Gas.ReadValue<float>();
            brakeInput = inputActions.CarPlaying.Brake.ReadValue<float>();

            //CAR DATA

            // We determine the speed of the car.
            carSpeed = carRigidbody.linearVelocity.magnitude; //(2 * Mathf.PI * frontLeftCollider.radius * frontLeftCollider.rpm * 60) / 1000;
            ...
            */
            ...
            if (gasInput == 0 && brakeInput == 0 && handbrakeInput == 0 && !deceleratingCar)
            {
        ...
        //The following method turns the front car wheels. The speed of this movement will depend on the steeringSpeed variable.
        public void Turn(float steering)
        {
            if (steering < 0)
            {
                steeringAxis = steering - (Time.deltaTime * steeringSpeed);
            }
            else if (steering > 0)
            {
                steeringAxis = steering + (Time.deltaTime * steeringSpeed);
            ...
            var steeringAngle = steeringAxis * steeringCurve.Evaluate(carSpeed);
            frontLeftCollider.steerAngle = Mathf.Lerp(frontLeftCollider.steerAngle, steeringAngle, 1 / steeringSpeed);
            frontRightCollider.steerAngle = Mathf.Lerp(frontRightCollider.steerAngle, steeringAngle, 1 / steeringSpeed);
        }
        ...
        public void ResetSteeringAngle()
        {
            if (steeringAxis < 0f)
            {
                steeringAxis = steeringAxis + (Time.deltaTime * steeringSpeed);
            }
        ...
        public void GoForward(float forwardInput)
        {
            CancelInvoke("DecelerateCar");
            deceleratingCar = false;
            ...
            else
            {
                ...
                {
                    //Apply positive torque in all wheels to go in reverse if maxReverseSpeed has not been reached.
                    if (_driveType == DriveType.AWD)
                    {
                        frontLeftCollider.motorTorque = (motorTorque) * throttleAxis;
                        frontRightCollider.motorTorque = (motorTorque) * throttleAxis;

                        rearLeftCollider.motorTorque = (motorTorque) * throttleAxis;
                        rearRightCollider.motorTorque = (motorTorque) * throttleAxis;
                    }
                    else if (_driveType == DriveType.RWD)
                    {
                        rearLeftCollider.motorTorque = (motorTorque) * throttleAxis;
                        rearRightCollider.motorTorque = (motorTorque) * throttleAxis;
                    }
                    else if (_driveType == DriveType.FWD)
                    {
                        frontLeftCollider.motorTorque = (motorTorque) * throttleAxis;
                        frontRightCollider.motorTorque = (motorTorque) * throttleAxis;
                    }
        ...
        public void GoReverse(float backwardInput)
        {
            CancelInvoke("DecelerateCar");
            deceleratingCar = false;
        ...
        public void Brakes(float brakeStrenght)
        {
            Debug.Log($"braking {brakeStrenght} -- {brakeForce * brakeStrenght}");
            leftBrakeLight.SetActive(true);
            rightBrakeLight.SetActive(true);

            frontLeftCollider.brakeTorque = brakeForce * brakeStrenght;
            frontRightCollider.brakeTorque = brakeForce * brakeStrenght;
        }
        public void EBrakes(float brakeStrenght)
        {
            rearLeftCollider.brakeTorque = brakeForce * brakeStrenght;
            rearRightCollider.brakeTorque = brakeForce * brakeStrenght;
        }
        public void ZeroBrakes()
        {
            leftBrakeLight.SetActive(false);
            rightBrakeLight.SetActive(false);
        ...
        public void Handbrake(float handbrake)
        {
            CancelInvoke("RecoverTraction");
            EBrakes(handbrake * .2f);
            ...
            if (useEffects)
            {
                try
                {
                    if (isDrifting)
                    {
                        ...
                        currentCamFollow.TrackerSettings.PositionDamping.x = Mathf.Lerp(2, 10, 1);
                    }
                    else //if (!isDrifting)
                    {
                        RLWParticleSystem.Stop();
                        RRWParticleSystem.Stop();
                        currentCamFollow.TrackerSettings.PositionDamping.x = Mathf.Lerp(10, 2, 1);
                    }
                }
                catch (Exception ex)
                {
